...
def conv3x3(in_planes, out_planes, stride=1):    # 辅助函数
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1):
        ...
        self.shortcut = nn.Sequential()                         # 初始化一个空的顺序容器, 用于定义捷径
    ...
    def forward(self, x):
        ...
        out += self.shortcut(x)                     # 将结果 out 与捷径路径的输出相加, 实现残差连接
        out = F.relu(out)
        return out
...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1):
        ...
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion * out_planes:      # 检查是否需要调整捷径路径
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion * out_planes, 1, stride=stride, bias=False)
            )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):       # 构造函数，初始化类的实例; block: 模块类型(如 BasicBlock 或 Bottleneck); num_blocks: 每个层的块数列表; num_classes: 输出的类别数，默认值为 10
        super(ResNet, self).__init__()
        self.in_planes = 64

        self.conv1 = conv3x3(3, 64)
        self.bn1 = nn.BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)        # 定义第一个残差层，由 num_blocks[0] 个 block 组成，输出通道数为 64，步幅为 1
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))                               # 定义自适应平均池化层, 将每个通道的特征图大小调整为 1x1

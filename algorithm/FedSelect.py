...
def FedSelect(args, logger):
    ...
    # for computation time and communication size
    ...
    for epoch in range(0, args.epochs):
        ...
        if bool(args.select_client) is True and epoch >= args.warmup_epochs and epoch != 0:
            ...
            client_weight = torch.ones(args.client_num, device=device)

            for k in range(args.client_num):
                client_weight[k] = weight_sum[k] / (len(client_data_loader[k]['train'].dataset))
        ...
        max_index = torch.argmax(total_value_list)
        ...
        meta_model_train(client_data_loader[client_i]['train'], client_data_loader[client_i]['reward_data'], client_model_list[client_i], client_meta_model, epoch, args, server_meta_optimizer)
        ...
        if accuracy_current > best_acc:
            ...
            if not os.path.exists(f'./saved_model/{args.algorithm}/{args.dataset_name}'):
                os.makedirs(f'./saved_model/{args.algorithm}/{args.dataset_name}')
            torch.save(server_model.state_dict(), f'./saved_model/{args.algorithm}/{args.dataset_name}/{args.algorithm}_best_server_model.pth')
        else:
        ...
        if bool(args.calculate_efficiency) is True:
            if accuracy_current >= args.target_accuracy:
                successive_accuracy_count += 1
                # if time_to_accuracy is None:
                #     time_to_accuracy = total_client_training_time + total_aggregation_time
                if successive_accuracy_count >= required_successive_count:
                    print(
                        f"连续 {required_successive_count} 个轮次达到目标精度 {args.target_accuracy}，停止训练")
                    break
            else:
                successive_accuracy_count = 0
    ...
    if bool(args.calculate_efficiency) is True:
        print("--------------End----------------")
        # print("--------------Efficient experiment--------------")
        print("final_best_acc_epoch: ", best_acc_epoch)
        print("final_best_acc: ", best_acc)
        print(f"Target accuracy: {args.target_accuracy} ")
        print(f"Total training time: {computation_time:.4f}s")
        print(f"Total communication size: {communication_size:.4f}GB")

        with open(f"./logs/{args.dataset_name}/{args.algorithm}/{args.algorithm}_{args.target_accuracy}.txt", "a") as f:
            f.write(
                f"------------------- Efficiency Experiments Summary ----------------------:\n"
                f"Seed: {args.seed}\n"
                f"Final best accuracy epoch: {best_acc_epoch}\n"
                f"Final best accuracy: {best_acc}\n"
                f"Target accuracy: {args.target_accuracy}\n "
                f"Total computation time: {computation_time:.4f}s\n"
                f"Total communication size: {communication_size:.4f}GB"
            )
        ...
        with open(f"./logs/{args.dataset_name}/{args.algorithm}/{args.algorithm}_final.txt", "a") as f:
            f.write(
                f"------------------- Effectiveness Experiments Summary ----------------------:\n"
                f"Seed: {args.seed}\n"
                f"Final best accuracy epoch: {best_acc_epoch}\n"
                f"Final best accuracy: {best_acc}\n"
            )
        with open(f"./logs/{args.dataset_name}/{args.algorithm}/{args.algorithm}.txt", "a") as f:
            f.write(
                f"Seed:{args.seed}\t"
                f"Final best accuracy epoch: {best_acc_epoch}\t"
                f"Final best accuracy: {best_acc}\n"

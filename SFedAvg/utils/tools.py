...
from SFedAvg.utils.sampling import mnist_iid, mnist_noniid, mnist_noniid_unequal, cifar_iid, cifar_noniid, \
    FashionMnist_noniid, cifar_longtail
from SFedAvg.utils.options import args_parser
...
def get_dataset(args):
    ...
    if args.dataset_name == 'cifar10':
        ...
        if args.iid:
            ...
            user_groups = cifar_iid(train_dataset, args.client_num)
        else:
            # Long-tailed
            if args.noise == 1:
                user_groups = cifar_longtail(train_dataset,  args.client_num, args.noiselevel)
            elif args.unequal:
                # Chose uneuqal splits for every user
                raise NotImplementedError()
            else:
                # Chose equal splits for every user
                user_groups = cifar_noniid(train_dataset, args.client_num)
        ...
        if args.iid:
            # Sample IID user data from Mnist
            user_groups = mnist_iid(train_dataset, args.client_num)
        else:
            # Sample 0_NonIID user data from Mnist
            if args.unequal:
                # Chose uneuqal splits for every user
                user_groups = mnist_noniid_unequal(train_dataset, args.client_num)
            else:
                # Chose euqal splits for every user
                user_groups = mnist_noniid(train_dataset, args.client_num)
        ...
        if args.iid:
            ...
            # Sample 0_NonIID user data from Mnist
                ...
                # Chose euqal splits for every user
                user_groups = FashionMnist_noniid(train_dataset, args.client_num)
...
def add_gradient_noise(args, w, idxs):
    ...
    for i in range(len(w)):
        for key in w[i].keys():
            if idxs[i] % 4 == 0:
                if args.noise == 5:
                    noise = torch.tensor(np.random.normal(0, args.noiselevel, w[i][key].shape)).to(device)

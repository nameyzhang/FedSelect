...
# sorted_id 是排序后的列表, 包含两个值: (2476, tensor(1.3256))
#   第一个值: 样本的索引k
#   第二个值: 对应样本的损失值


# list_loss_benchmark: [损失值1, 损失值2, ..., 损失值M]
...
#     return dict_users
def wash_data(dataset, indices_to_keep):
    # 更新 dict_users 中的用户数据
    # for user in dict_users:
        # 使用列表推导式来过滤掉不在 indices_to_keep 的索引
    # dict = [index for index in dict if index in indices_to_keep]
    # 使用 Subset 通过过滤后的索引创建新的数据集

...
class SamplesLoss(Module):
    ...
    def generate_weights(self, x):
        ...
        # 如果输入是 4D 张量 (B, C, H, W)，将其展平为 2D (B, N)，其中 N = C * H * W
        if x.dim() == 4:
            B, C, H, W = x.shape
            x = x.view(B, -1)  # 将 (C, H, W) 展平为 (N)
    ...
    #         x = x.view(B, -1)  # 将 (C, H, W) 展平为 (N)
    #
    #     # 检查 x 是否符合 (N,D) 或 (B,N,D) 的维度要求
    #     if x.dim() == 1:  # 如果 x 是 1D 数组，增加一个维度，使其变为 (N, 1)
    #         x = x.unsqueeze(1)
    #
    #     if x.dim() == 2:  # (N,D) 格式
    #         N = x.shape[0]
    #         return torch.ones(N).type_as(x) / N
    #
    #     elif x.dim() == 3:  # (B,N,D) 格式
    #         B, N, _ = x.shape
    #         return torch.ones(B, N).type_as(x) / N
    #
    #     else:
    #         raise ValueError(
    #             f"Invalid input shape: {x.shape}. Input samples 'x' should be encoded as (N,D) or (B,N,D) (batch) tensors."
    #         )

    def check_shapes(self, l_x, α, x, l_y, β, y):
        # 打印输入维度，帮助调试
        # print(f"x shape: {x.shape}, y shape: {y.shape}, α shape: {α.shape}, β shape: {β.shape}")



        def flatten_if_necessary(x, y):
            # 打印输入形状，帮助调试
            # print(f"x shape: {x.shape}, y shape: {y.shape}")

            # 展平 x 和 y 的形状，但保持各自的批次大小
            if x.dim() == 4:  # 4D (B, C, H, W) 输入
                B_x, C_x, H_x, W_x = x.shape
                x = x.view(B_x, -1)  # 展平为 (B, C * H * W)

            if y.dim() == 4:  # 同样处理 y
                B_y, C_y, H_y, W_y = y.shape
                y = y.view(B_y, -1)  # 使用 y 自己的批次大小 B_y 进行展平

            return x, y


        # 调整 x 和 y 的维度
        x, y = flatten_if_necessary(x, y)
